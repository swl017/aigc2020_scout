...
void check_WPT()
{
  ...
  PtGx = int((Goal[CurrentGoal][0]-MapInfo_origin_x)/(MapInfo_resolution));
  PtGy = int((Goal[CurrentGoal][1]-MapInfo_origin_y)/(MapInfo_resolution));
  if(CurrentGoal<(GOALNUM-1))
  {
    if(sqrt(float((PtGx-PtSx)*(PtGx-PtSx)+(PtGy-PtSy)*(PtGy-PtSy)))<AcceptanceRadius/MapInfo_resolution)
    {
      CurrentGoal++;
      PtGx = int((Goal[CurrentGoal][0]-MapInfo_origin_x)/(MapInfo_resolution));
      PtGy = int((Goal[CurrentGoal][1]-MapInfo_origin_y)/(MapInfo_resolution));
      ROS_INFO("Moving to Next Goal! %f, %f", Goal[CurrentGoal][0], Goal[CurrentGoal][1]);
    }
  }
  else
  {
    if(sqrt(float((PtGx-PtSx)*(PtGx-PtSx)+(PtGy-PtSy)*(PtGy-PtSy)))<AcceptanceRadius/MapInfo_resolution)
    PtGx = int((Goal[GOALNUM-1][0]-MapInfo_origin_x)/(MapInfo_resolution));
    PtGy = int((Goal[GOALNUM-1][1]-MapInfo_origin_y)/(MapInfo_resolution));
    ROS_INFO("Reached Endpoint!");
  }
  //ROS_INFO("Current Goal = %d", CurrentGoal);
}
