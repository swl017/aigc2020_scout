...
double* GenPath(int sx1, int sy1, int gx1, int gy1, int **obs_map, int MaxX, int MaxY )
{
	...
	else
	{
		...
		{
			waypoint[2*i+1] = path1_wayx[i] ;
			waypoint[2*i+2] = path1_wayy[i] ;
		}
...
//	If not already on the c losed list (items on the closed list have
...
//	Don't cut across corners
	...
	if (corner == walkable) {
		...
		if (whichList[a][b] != onOpenList)
		{
			...
			//Move the new ope list item to the proper place in the binary heap.
